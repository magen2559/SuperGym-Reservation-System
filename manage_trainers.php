    ...
    if (empty($name) || empty($email) || empty($password)) {
        ...
        if ($stmt->fetch()) {
            ...
            try {
                ...
                $stmt = $pdo->prepare("INSERT INTO trainers (user_id, specialty, bio) VALUES (?, ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_trainer'])) {
    ...
    if (empty($name) || empty($email)) {
        ...
        $pdo->beginTransaction();
        try {
            $stmt = $pdo->prepare("UPDATE users SET name = ?, email = ? WHERE id = ? AND role = 'trainer'");
            $stmt->execute([$name, $email, $user_id]);
            
            $stmt = $pdo->prepare("UPDATE trainers SET specialty = ?, bio = ? WHERE user_id = ?");
            $stmt->execute([$specialty, $bio, $user_id]);
...
$query = "
    SELECT u.id, u.name, u.email, u.created_at, 
          t.trainer_id as trainer_id, t.specialty, t.bio
    FROM users u
...
<head>
    ...
    <title>SuperGym - Manage Trainers</title>
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" name="add_trainer" class="btn btn-primary-custom">Add Trainer</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" name="edit_trainer" class="btn btn-primary-custom">Save Changes</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    editTrainerModal.addEventListener('show.bs.modal', function(event) {
